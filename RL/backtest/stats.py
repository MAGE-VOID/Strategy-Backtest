...
class Statistics:
    def __init__(self, all_trades, equity_over_time, initial_balance):
        """
        Parámetros:
          - all_trades: lista de diccionarios con información de cada evento de trade (apertura y cierre).
          - equity_over_time: lista de diccionarios con la evolución de la equidad.
          - initial_balance: balance inicial.
        """
        # Filtrar solo los trades cerrados (que contienen 'profit')
    ...
    def _calculate_drawdowns(self):
        equity_array = np.array([rec["equity"] for rec in self.equity_over_time])
    ...
    def _calculate_var(self, final_balance):
        equity_values = np.array([rec["equity"] for rec in self.equity_over_time])
    ...
    def calculate_statistics(self):
        ...
        # Equity over time
        equity_vals = np.array([rec["equity"] for rec in self.equity_over_time])
        min_eq, max_eq = equity_vals.min(), equity_vals.max()
        ...
        # CAGR corregido (solo si growth > 0)
        growth = final_balance / self.initial_balance
        ...
        log_returns = np.diff(np.log(np.clip(equity_vals, 1e-6, None)))
        if log_returns.size > 0:
            avg_period = duration.total_seconds() / (len(equity_vals) - 1)
        ...
        profits_array = np.array([t["profit"] for t in self.closed_trades])
